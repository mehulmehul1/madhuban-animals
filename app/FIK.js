...
(function (global, factory) {
	...
	class M3 {
		...
		get XBasis() {
			return new V3(this.elements[0], this.elements[1], this.elements[2]);
		}
		...
		get YBasis() {
			return new V3(this.elements[3], this.elements[4], this.elements[5]);
		}
		...
		get ZBasis() {
			return new V3(this.elements[6], this.elements[7], this.elements[8]);
		}
		...
		rotateAboutAxis(v, angle, axis) {
			let sin = Math.sin(angle);
			let cos = Math.cos(angle);
			let oneMinuscos = 1.0 - cos; // It's quicker to pre-calc these and reuse than calculate x * y, then y * x later (same thing).
			...
			te[6] = xzOne - axis.y * sin; // Calculate rotated y-axis

			te[1] = xyOne - axis.z * sin;
			te[4] = axis.y * axis.y * oneMinuscos + cos;
			te[7] = yzOne + axis.x * sin; // Calculate rotated z-axis

			te[2] = xzOne + axis.y * sin;
			te[5] = yzOne - axis.x * sin;
			te[8] = axis.z * axis.z * oneMinuscos + cos; // this.identity()
	...
	class Structure3D {
		...
		addBoneMesh(bone, prev, ar, chain) {
			...
			const axis = new this.THREE.AxesHelper(2); //let bw = new this.THREE.Mesh( g,	m4 );
			...
			if (prev !== -1) {
				if (extraMesh !== null) {
					if (type !== J_GLOBAL) {
						...
						b.userData.axe = axe; //extraMesh.position=(b.position)
				...
				if (extraMesh !== null) {
					b.add(extraMesh);
					b.userData.extra = extraMesh;
					b.userData.axe = new this.THREE.Vector3(0, 0, 1); //extraMesh.position=(b.position)
					//this.scene.add( extraMesh )
				}
			}
